...
function DetailView({ id, onBack, onChangeWorker }: { id: string; onBack: () => void; onChangeWorker: (contract: Contract) => void }) {
  ...
  const showQr = async () => {
    if (!data) return;
    try {
      Toast.show({ icon: 'loading', content: '正在生成收款码...', duration: 0 });
      const res = await contractService.generatePaymentQr(data._id);
      Toast.clear();
      Dialog.show({
        title: `收款 ${fmtCents(res.amount)}${res.label ? `（${res.label}）` : ''}`,
        content: (
        ...
        closeOnMaskClick: true,
        actions: [{ key: 'close', text: '关闭', onClick: () => { reload(); } }],
      });
    } catch (error) {
      Toast.clear();
      Toast.show({ icon: 'fail', content: '生成收款码失败' });
    }
  };
