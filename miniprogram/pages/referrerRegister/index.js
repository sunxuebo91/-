Page({
  ...
  async onLoad(options) {
    ...
    if (options.scene) {
      const scene = decodeURIComponent(options.scene); // e.g. "p=186...&cid=xxx"
    ...
    // 持久化，防止扫码后被中断再回来时丢失（staffId 和 customerId 都需要保存）
    ...
    this.setData({ sourceStaffId: staffId, sourceCustomerId: customerId });
  ...
  async _doRegister(phone) {
    const { name, sourceStaffId, sourceCustomerId } = this.data;
    this.setData({ submitting: true });
    try {
      const res = await wx.cloud.callFunction({
        name: 'referralService',
        data: {
          ...
          phone,
          sourceStaffId,
          sourceCustomerId,  // 关联客户订单 ID
        },
      });

      const result = res && res.result;
      console.log('[registerReferrer] success=', result && result.success, '_id=', result && result.data && result.data._id);
